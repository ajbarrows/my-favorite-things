...
echo "Checking what will be pushed to vacc:$PROJECT_PATH/..."
ALL_CHANGES=""
...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ ! -e "$PROJECT_PATH/$line" ]; then
        continue
    fi

    # Run dry-run for this subfolder (with --delete to show what would be removed)
    CHANGES=$(rsync -avn --itemize-changes --delete $PROJECT_PATH/$line vacc:$PROJECT_PATH/$line 2>/dev/null | grep -v '/$')
...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ -e "$PROJECT_PATH/$line" ]; then
        echo "Syncing: $line"
        rsync -avP $DELETE_FLAG $PROJECT_PATH/$line vacc:$PROJECT_PATH/$line
    fi
done < "$SYNC_CONFIG"
